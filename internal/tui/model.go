...
type model struct {
	...
	promptHistoryEntries  []storage.PromptEntry
	promptSearchMode      promptSearchMode
	promptSearchQuery     string
	promptSearchMatches   []storage.PromptEntry
...
func (m *model) ensurePromptHistoryLoaded() {
	...
	entries, err := storage.LoadRecentPrompts(promptSearchLoadLimit)
	if err != nil {
...
func (m *model) refreshPromptSearchMatches() {
	...
	matches := make([]storage.PromptEntry, 0, min(len(m.promptHistoryEntries), limit))
...
func (m model) selectedPromptSearchEntry() (storage.PromptEntry, bool) {
	if len(m.promptSearchMatches) == 0 {
		return storage.PromptEntry{}, false
	}
	index := clamp(m.promptSearchCursor, 0, len(m.promptSearchMatches)-1)
	return m.promptSearchMatches[index], true
}

func (m model) visiblePromptSearchEntriesPage() []storage.PromptEntry {
	if len(m.promptSearchMatches) == 0 {
		return nil
	}
