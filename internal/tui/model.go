...
const (
	...
	footerHintText             = "tab agents | / commands | drag select | Ctrl+C copy/quit | Ctrl+L sessions"
...
type model struct {
	...
	promptSearchMatches   []storagepkg.PromptEntry
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case runFinishedMsg:
		...
		case runFinishReasonFailed:
			m.streamingIndex = -1
			detail := describeProviderError(msg.Err.Error(), m.cfg.Provider.Model)
			m.statusNote = "Run failed: " + compact(detail, 180)
			m.phase = "error"
			m.llmConnected = false
			m.failLatestAssistant(detail)
		default:
			m.streamingIndex = -1
			m.statusNote = "Ready."
			m.phase = "idle"
...
func (m model) handleKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	if msg.String() == "enter" && !msg.Paste {
		...
		// Startup setup should submit immediately. Do not run paste-suppression
		// or long-paste compression before applying provider fields/API key.
...
func (m *model) ensurePromptHistoryLoaded() {
	...
	entries, err := storagepkg.LoadRecentPrompts(promptSearchLoadLimit)
	if err != nil {
...
func (m *model) refreshPromptSearchMatches() {
	...
	m.promptSearchMatches = storagepkg.FilterPromptEntries(m.promptHistoryEntries, m.promptSearchQuery, limit)
	matches := m.promptSearchMatches
	if len(matches) == 0 {
		m.promptSearchCursor = 0
		return
	}
...
func describeProviderError(errText, model string) string {
	...
	hint := fmt.Sprintf("Configured model %q is unavailable on this provider. Update provider.model, then retry.", model)
	return hint + " Raw error: " + compact(errText, 260)
}

func (m *model) failLatestAssistant(errText string) {
	errText = describeProviderError(errText, m.cfg.Provider.Model)
	if len(m.chatItems) == 0 {
		m.appendChat(chatEntry{
			Kind:   "assistant",
			Title:  assistantLabel,
...
func (m model) selectedPromptSearchEntry() (storagepkg.PromptEntry, bool) {
	if len(m.promptSearchMatches) == 0 {
		return storagepkg.PromptEntry{}, false
	}
	index := clamp(m.promptSearchCursor, 0, len(m.promptSearchMatches)-1)
	return m.promptSearchMatches[index], true
}

func (m model) visiblePromptSearchEntriesPage() []storagepkg.PromptEntry {
	if len(m.promptSearchMatches) == 0 {
		return nil
	}
...
func (m model) helpText() string {
	return strings.Join([]string{
		...
		"Prompt search is available in internal workflows and command flows when opened by the app.",
		...
		"The footer keeps only the essential shortcuts: tab agents, / commands, drag select, Ctrl+C copy/quit, Ctrl+L sessions.",
	}, "\n")
}
